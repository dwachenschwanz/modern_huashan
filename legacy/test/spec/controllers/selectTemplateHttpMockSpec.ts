...
describe('Controller: selectTemplateController (HTTP Mocking)', function () {
    ...
    scope,
    injector,
    location, flush, controller, httpBackend, templates, myTemplateResponse;
    ...
    it('template names should match', function() {
        var templateNames: Array<string> = ["salesForecast", "drugplatform", "acquisition"];
    ...
    it("should have the platform tag", function() {
        var expected:Array<boolean> = [false, true, false];
        for (var i=0;i<templates.length;i++) {
    ...
    it("should delete templates properly", function() {
        ...
        httpBackend.when('GET', 'https://localhost/kirk/wizard/main?command=DeleteTemplate&kreds=fakeCredentials&templateName=acquisition'
            ).respond(deleteTemplateResponse());
