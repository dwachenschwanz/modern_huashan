 ...
 *
 * Vendored unchanged from the legacy app (app/scripts/lib/core/md5-min.js),
 * wrapped as an ES module exporting only hex_md5, which is what the app uses.
 */
...
function binl2rstr(b) {
  let a = '';
  for (let c = 0; c < b.length * 32; c += 8) a += String.fromCharCode((b[c >> 5] >>> (c % 32)) & 255);
  return a;
}
...
function binl_md5(p, k) {
  p[k >> 5] |= 128 << (k % 32);
  p[(((k + 64) >>> 9) << 4) + 14] = k;
  let o = 1732584193, n = -271733879, m = -1732584194, l = 271733878;
  for (let g = 0; g < p.length; g += 16) {
    const j = o, h = n, f = m, e = l;
